  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {data && (
          <div className="ml-auto flex items-center gap-2 text-[11px] text-stone-500">
            <Chip>{data.games.length} games</Chip>
      ...
      {data && data.games.length === 0 && (
        <div className="rounded border border-stone-200 bg-white p-4 text-stone-500">No games scheduled for week {shown}.</div>
      )}
              ...
              {days.map((d) => (
                <Fragment key={d.label}>
                  <tr className="border-t border-stone-200 bg-stone-50">
                    <td colSpan={5} className="px-3 py-1 text-[11px] font-semibold text-stone-600">{d.label}</td>
                  </tr>
                  {d.games.map((g) => <GameRow key={g.game_id} g={g} />)}
                </Fragment>
              ))}
            </tbody>
          </table>
